...
export default class LastUpdatedText extends React.Component {
  ...
  render() {
    ...
    const hoursText =
      this.state.timeDifferenceInHours > 0
        ? `${this.state.timeDifferenceInHours} hour${hoursPostfix} `
        : "";

    return (
      <div>
        Last updated: {hoursText}
        {this.state.timeDifferenceInMinutes} minute{minutesPostfix} ago
      </div>
    );
  }
}
